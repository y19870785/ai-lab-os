...
> PILOT-001-IB-IMP-A 已验证普通 child 不继承匿名 capability，但同 UID 进程可经 `pidfd_getfd` 复制并调用 endpoint；因此 signing-oracle isolation 失败，Spike 最终分类为 `UNSUPPORTED`，未授权 Bridge 实现。
