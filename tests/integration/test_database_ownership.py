...
async def test_composition_root_injects_one_manager_and_preserves_paths(tmp_path: Path):
    ...
    await system.start()
    try:
        assert system.database_manager.connection_count == 7
        ...
        assert system.database_manager.get_path("inbox") == (
            settings.sqlite_dir / "inbox.db"
        ).resolve()
        assert system.database_manager.get_path("waiting_for") == (
            settings.sqlite_dir / "followups.db"
        ).resolve()
        assert system.database_manager.get_path("interactions") == (
            settings.sqlite_dir / "interactions.db"
        ).resolve()
...
async def test_system_shutdown_closes_every_managed_connection(tmp_path: Path):
    ...
    connections = [
        ...
        for name in (
            "episodic", "semantic", "decision", "user_tasks", "inbox", "waiting_for",
            "interactions",
        )
    ]
