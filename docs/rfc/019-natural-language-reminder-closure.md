...
Explicit task forms such as `添加任务：...` create only a UserTask. Explicit reminder markers require a supported future time and never degrade to task-only success.
...
The API accepts `Idempotency-Key` (or the request field). Only a SHA-256 hash is persisted. A stable UserTask ID is derived from workspace scope and that hash. The same key and normalized intent return the existing chain; the same key with a different intent returns `reminder.idempotency_conflict`. Titles are not globally deduplicated.
