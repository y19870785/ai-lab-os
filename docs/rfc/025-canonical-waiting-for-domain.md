...
SP-016 为这些外部依赖建立独立真相源，并让 Daily Agenda 在可选能力关闭时仍能聚合其他可用来源。本 RFC 的模型、持久化、生命周期及入口边界已由实现和自动化测试验证。
