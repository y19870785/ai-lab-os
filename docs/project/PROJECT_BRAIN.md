...
`pyproject.toml` 的 `[project].version` 是唯一运行时产品版本来源。v0.33.0 是稳定化发布基线。SP-004 建立 Canonical UserTask 与真实 Task API，但不创建新 Tag；Reminder/UserTask-Scheduler Bridge 留给 SP-005。
...
SP-004 开发分支的 Windows 本地完整验证为 `847 passed, 27 warnings in 38.81s`，该数字不回写为 v0.33.0 发布基线，也不是 GitHub Actions 结果。
...
冻结标签保持不变。SP-001、SP-001A、SP-002 与 SP-003 均已完成审查并进入 `main`。SP-003 经 PR #5 以 Squash Merge 合并，审查结论为 `APPROVED`；SP-003 merge baseline 为 `ce3655ff5f7a625da6b168058873dadfc2289b5f`。
...
3. v0.33.0 作为 SP-001 至 SP-003 的产品稳定化基线，产品版本由 `pyproject.toml` 唯一管理。
4. Reminder/UserTask-Scheduler、Knowledge Reindex/Chunk Persistence/Citation、自动 Tool Calling、Coordination 主链路、Database backup/restore 与 shutdown 全局请求闸门仍未完成。
5. 在主链路稳定前不推进新的产品 Phase。
