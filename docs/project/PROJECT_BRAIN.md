...
> SP-003 Status：Implemented on branch / Awaiting review
...
| DatabaseManager | Integrated / Verified | SP-003 已在独立分支实现：管理三类 Memory SQLite 共享连接、路径绑定与统一关闭，等待审查 |
...
2. SP-003 DatabaseManager Connection Ownership 已在独立分支实现，当前只等待真实 Diff 审查，不得描述为已合并或 Completed。
3. Reminder/UserTask-Scheduler、Knowledge Reindex、自动 Tool Calling 和 Coordination 扩展仍未完成。
4. 在主链路稳定前不推进新的产品 Phase。

## SP-003 范围边界

SP-003 只收敛 Episodic、Semantic、Decision 三类 Memory SQLite 连接所有权。Manager 是 Managed Mode 唯一 Owner，Store 不关闭 borrowed connection；Standalone Mode 继续使用并关闭自身连接。现有数据库路径和 Schema 不变。Knowledge SQLite Store、SchedulerPersistence 以及 shutdown 期间全局拒绝新数据库操作的闸门仍未纳入本轮。

SP-003 分支本地验证记录：专项测试 `27 passed in 1.22s`，受影响模块 `145 passed in 8.29s`，全量测试 `795 passed, 26 warnings in 37.05s`。全量首跑因测试进程继承 SOCKS 代理出现 5 个既有 DeepSeek real 测试错误；在测试子进程清空代理变量后全部通过，未修改用户全局环境。该结果不是 GitHub Actions 记录。
