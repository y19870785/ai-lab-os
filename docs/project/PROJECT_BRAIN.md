...
> SP-014 Status：APPROVED / MERGED / MANUAL_ACCEPTANCE_PASSED / RECONCILED / ARCHIVED
> SP-014B Status：APPROVED / MERGED / VERIFIED / RECONCILED / ARCHIVED
> ACC-014 Status：PASSED / FINAL
> SP-015 Status：UNBLOCKED_FOR_PLANNING / NOT_STARTED
> Current main：`22f85db16a43e7d09a903859a26ac6a310370d81`
> Last completed SP：SP-014
> Next SP：SP-015（未启动）

## SP-014 永久产品事实

- Unified Inbox 与 Capture-to-Action 最小闭环已通过 PR #32 以 Squash Commit `5bad5d412f9f2dabb158527a96c20c6e95e86d6e` 进入 `main`。
- API、CLI 与 CEO Assistant 共用 canonical Composition Root 注入的 `InboxService`，支持转化为 UserTask、Reminder、Work Log、Note，或显式 Dismiss。
- `inbox_resolution_claims` 是跨 API worker、CLI 进程与独立 Service 实例的唯一解析权和崩溃恢复边界；进程内锁不是正确性来源。
- Workspace 隔离、幂等、不同类型竞争、同类型竞争、claim 后中断恢复和目标创建后中断恢复均通过真实验收。
- SP-014B 通过 PR #33 以 Squash Commit `22f85db16a43e7d09a903859a26ac6a310370d81` 合入，确定性 Reminder Parser 在明确 `上午/下午/晚上` 时支持中文小时 `一` 至 `十二`，不扩展复杂日期或 LLM 时间解析。
- ACC-014 A～L 全部 PASSED；SP-014 与 SP-014B 已对账封存。SP-015 仅解除规划阻塞，尚未设计、批准或启动。
...
| Unified Inbox | Integrated / Verified | SP-014 已合并并完成 A～L 手工验收；持久化 resolution claim 提供跨进程唯一解析权与恢复边界 |
...
5. SP-014 Unified Inbox 与 SP-014B 中文小时兼容修复已完成、验收并封存；下一项仅为 SP-015 `UNBLOCKED_FOR_PLANNING / NOT_STARTED`。
6. 外部通知渠道、Recurring Reminder、Knowledge Reindex/Chunk Persistence/Citation、完整 Agent Runtime 产品闭环、自动 Tool Calling、完整 MCP 闭环、Coordination 主链路、UI、Database backup/restore、in-flight counting 与 drain timeout 仍未完成。
...
- SP-014: `APPROVED / MERGED / MANUAL_ACCEPTANCE_PASSED / RECONCILED / ARCHIVED`；ACC-014 A～L 全部通过。
- SP-015: `UNBLOCKED_FOR_PLANNING / NOT_STARTED`；尚未定义任务书或启动实现。
