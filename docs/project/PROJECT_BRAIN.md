...
- Global shutdown request gate: not implemented
...
- Accepted same-Task work may finish after DRAINING; ordinary detached children must be readmitted and cannot reuse copied ContextVar state.
- Scheduler is the only explicit spawned-continuation exception through `spawn_accepted_task()`; rejected DRAINING ticks create no claim, JobRun, executor call, or background task.
- FastAPI `get_runtime()` depends on guarded `get_system()`; `ApplicationRuntime.execute()` performs the canonical execution-time check. Business routes cannot import an unguarded resolver.
- Task, Workflow, Agent, health, recovery, startup, and shutdown are not repeatedly gated.
