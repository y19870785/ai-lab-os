...
> Next Candidate SP: SP-017
> Next Candidate Direction: Follow-up Interaction & Capture Closure
> SP-016 Status: APPROVED / IMPLEMENTATION_COMPLETE / AUTOMATED_VERIFICATION_PASSED / MANUAL_ACCEPTANCE_PENDING
...
| Waiting-For | 独立 canonical domain、`followups.db`、API/CLI 与 append-only history 已完成自动化验证；人工验收待执行 |
...
| Waiting-For | Integrated / Automated verification passed | SP-016 人工验收待执行 |
...
SP-016 Canonical Waiting-For Domain & Agenda Closure 已批准、实现完成并通过自动化验证，人工验收待执行；当前治理任务为 None。SP-017 Follow-up Interaction & Capture Closure、SP-018 Structured Work Log Query & Context Linking 与 SP-019 Daily Review & Follow-up Brief 仅为候选，均未批准、未启动。
