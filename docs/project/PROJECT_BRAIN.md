...
> 当前治理任务：None
> 当前工作：INT-001
> INT-001 状态：IMPLEMENTATION_AUTHORIZED / OPEN / DRAFT / PENDING_INDEPENDENT_REVIEW / NOT_READY / NOT_MERGE_AUTHORIZED
...
与 main Quality Gate，并完成 canonical domain 实施、治理对账和封存；INT-001 已获独立实现授权，
已建立 Shell-neutral Adapter、fail-closed authority 与本地 stdio MCP reference projection，当前仍为 Draft
并等待独立审查，未获 Ready 或 Merge 授权。PILOT-001 与 REL-036 均未启动。
...
SP-016～SP-021 均已完成验收并封存；ACC-016～021 均为 PASSED / FINAL。Latest Merged SP 与 Latest Completed SP 均为 SP-021；Current Product SP 与 Current Governance Task 均为 None。当前工作 INT-001 已获实现授权并处于 Draft / PENDING_INDEPENDENT_REVIEW / NOT_READY / NOT_MERGE_AUTHORIZED；真实 Hermes/Channel 未接入。当前产品版本仍为 `0.35.0` Alpha GitHub Pre-release；该发布不等于 production-ready。
