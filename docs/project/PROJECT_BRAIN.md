...
> SP-018 Status: PLANNING_BASELINE_DEFINED / IMPLEMENTATION_NOT_APPROVED / NOT_STARTED
> ACC-018 Status: PLANNING_BASELINE / NOT_EXECUTED / IMPLEMENTATION_NOT_APPROVED
> SP-018 Design: RFC-027 Proposed / Planning Baseline; ADR-058, ADR-059 and ADR-060 Accepted
> Current main: `dbad2bb230dcd22ebc5134c3c432dd6746c74a08` / Quality Gate run `30009015891`
...
### SP-018 Planning facts / not implemented

- SP-018 计划建立唯一 `WorkLogService`，但尚未批准或开始实施。
- `episodic.db / episodic_memories` 计划继续作为物理存储；不会创建 `work_logs.db` 或第二张 Work Log 表。
- Work Log 查询计划要求完整 `tenant_id + workspace_id + namespace` WorkspaceKey。
- 新记录计划使用 `wl_...` canonical ID；旧随机 Memory ID 计划通过确定性 `wl_legacy_...` 只读投影兼容，不写回。
- 上下文关联只计划接受显式 `ut_`、`rem_`、`wf_`、`inbox_` ID；不使用 LLM 或相似文本猜测。
- Daily Agenda 与 Daily Brief 计划统一通过 WorkLogService 读取，不再直接扫描通用 Memory。
- SP-019 必须等待 SP-018 实施、验收和封存完成；规划基线合并不自动启动任何实施。
...
SP-016 Canonical Waiting-For Domain & Agenda Closure 与 SP-017 Follow-up Interaction & Capture Closure 均已完成人工验收并封存；ACC-016、ACC-017 均为 PASSED / FINAL。Current Product SP 为 None，Current Governance Task 为 None。SP-018 已定义 Planning Baseline，但 IMPLEMENTATION_NOT_APPROVED / NOT_STARTED；RFC-027 为 Proposed / Planning Baseline，ADR-058～ADR-060 为 Accepted，ACC-018 为 NOT_EXECUTED。SP-019 仍为候选且未批准、未启动。
