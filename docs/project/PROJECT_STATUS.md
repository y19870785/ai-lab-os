...
当前产品版本仍为 v0.33.0，未创建 v0.34.0 Tag 或 GitHub Release。外部通知渠道、Recurring Reminder、Inbox、Knowledge Reindex/Chunk Persistence/Citation、自动 Tool Calling、完整 MCP 闭环、Coordination 主链路、UI、Database backup/restore、in-flight counting 与 drain timeout 仍未完成。
...
| Python Files | 456 |
| Code Lines | 29,259 |
| Tests | SP-009 candidate: 1006 passed, 27 warnings in 50.02s（Windows 本地验证，非跨平台 CI） |
| RFC | 20 |
| ADR | 41 |
...
SP-009 Natural-Language Reminder Closure with In-App Status 已进入 **implementation candidate / Draft PR / Awaiting ChatGPT review / Not merged**。候选范围只闭合受支持的今天/明天提醒、真实持久化调度、唯一 Occurrence 与站内状态查询；尚未成为已集成或已验证的 main 能力。
