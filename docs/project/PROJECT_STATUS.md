...
| SP-017 | PLANNING_BASELINE_DEFINED / NOT_APPROVED_FOR_IMPLEMENTATION / NOT_STARTED |
| RFC-026 | Proposed / Planning Baseline |
| ADR-056 / ADR-057 | Accepted（架构边界已接受，不代表实施获批） |
...
v0.34.0 Alpha 之上的 v0.35 开发线已实现 canonical Waiting-For 与 Daily Agenda 可选来源组合。Reminder 和 Scheduler 默认关闭时，Agenda 仍可读取其他已启用来源；Knowledge 和 Coordination 仍不是默认主链路。
...
Current Product SP 为 None，Current Governance Task 为 None，Latest Completed SP 为 SP-016。SP-016 已完成人工验收并封存，ACC-016 为 PASSED / FINAL。下一候选为 SP-017 — Follow-up Interaction & Capture Closure；其规划基线已定义，但仍未批准实施、未启动。当前产品版本仍为 `0.34.0`。
