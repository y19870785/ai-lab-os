...
传给 AI-Lab 的有效入站字段仍是模型可控 tool arguments。Ingress bridge 设计已经批准，但当前 Pilot WSL2 同 UID
拓扑允许 `pidfd_getfd` 调用 issuer authority，因此当前部署仍为 `UNSUPPORTED`；Option D 设计基线保留，process
...
PR #77 已以 `1daf52ee500d5dc79ba1fc632f240ddf756bb93b` Squash Merge；唯一父提交为
`89aaab92320f1541cabd36d5ef8d7b69b0f450e4`，main Quality Gate `31719553362 / SUCCESS`，
独立 Post-Merge Verification 已通过。P1A 的精确能力仍只适用于
`PILOT_GRADE_LOCAL_TRUSTED_HOST_PROFILE`：真实 WeCom Owner Message A/B → strict
TrustedIngressEvidence → Confirmation → Interaction `AUTHORIZED / NOT_STARTED`。它不自动授权
Bridge implementation、Phase 1 Full、Phase 2 或真实业务 mutation。
PILOT-001-P1B 已在真实 Ubuntu/WSL2 下验证进程本地 Non-Dumpable mitigation：最终 Hermes
Python 进程内 `PR_SET_DUMPABLE=0` + 运行时 verify 0，same-UID `pidfd_getfd` capability
复制与 issuer 调用均被拒绝（errno=1 EPERM，`/proc` 复制 errno=13 EACCES），restart 后
