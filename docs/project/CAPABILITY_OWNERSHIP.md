...
> SP-021 当前为 APPROVED / MERGED / MAIN_QUALITY_GATE_PASSED / POST_MERGE_RECONCILED / ARCHIVED。
> 它只实现 AI-Lab-owned canonical
> Interaction Domain；Shell、Channel 与真实 Execution Adapter 的所有权边界不变。
...
| `core/agent`、`core/agents` 与顶层 `agents` 重叠 | CANONICAL_OWNERSHIP_CONSOLIDATED / COMPATIBILITY_RETAINED | `core/agents` 是唯一 canonical 实现；`agents/` 与 `core/agent/` 因 base artifact import contract 暂作 deprecated compatibility，后者无一对一 forwarding 映射 | 至少保留 v0.36，最早 v0.37.0 另行授权删除 | 未完成最终 namespace removal |
| 重复 Knowledge 实现 | CANONICAL_OWNERSHIP_CONSOLIDATED / COMPATIBILITY_RETAINED | `core/knowledge` 是唯一 canonical 实现；顶层 `knowledge/` 保留原语义 deprecated compatibility package | 至少保留 v0.36，最早 v0.37.0 另行授权删除 | 未完成最终 namespace removal |
