...
> 当前源码版本：`v0.35.0` Alpha / GitHub Pre-release Published | 更新日期：2026-08-13
...
IB-IMP-A Approved Evidence Head `a50fc8719158c07a4eee716c3513e9698c8571ff` 已通过最终独立安全审查，negative evidence baseline 已批准。PILOT-001-P1B 已在真实 WSL2 下测试进程本地 Non-Dumpable mitigation（`PR_SET_DUMPABLE=0` 运行时验证），same-UID `pidfd_getfd` 复制与 issuer 调用被拒绝（errno=1 EPERM），restart 后仍成立；P1B-R2/R6 已用真实 Hermes Python + `gateway.run` 经 launcher 实际链路证明 actual Hermes runtime-entry（实际 Hermes `gateway/run.py` 文件内首个执行 frame 产生 `stage=module_started`、`evidence.pid == Popen.pid`、`dumpable=0`、进程稳定存活、内核拒绝跨进程 `/proc/<pid>/mem` 访问）。pre-call `dispatching` 不构成完成证据；内核观察仅 EACCES/EPERM 判为预期拒绝，wrong PID/module、dispatching-only、导入失败、提前退出、超时、路径冒充与其他 errno 均 fail closed；R1 的 stub 测试明确为 CONTROLLED_RUNTIME_LINK_VALIDATION。该结果仅覆盖 tested Pilot profile（RUNTIME_ENTRY_PROVEN，非完整服务生命周期），不改变 `SIGNING_ORACLE_ISOLATION_FAILED_FOR_TESTED_TOPOLOGY` 历史负面证据，也不把 `PRODUCTION_PROCESS_ISOLATION_UNRESOLVED` 变成 resolved。
...
| 企业微信 Owner Pilot 仅到内部授权 | P0-E/P0-R 已完成；P1A 在本机可信 host profile 下真实证明 Fresh Evidence + challenge 可把 Interaction 推进到 `AUTHORIZED / NOT_STARTED`，UserTask 为零增量。该 profile 明确排除 hostile same-UID process；IB-IMP-A 的 `pidfd_getfd` signing-oracle isolation 失败仍成立。默认 live plugin 继续不存在，生产/通用 Fresh Owner Evidence 仍为 `UNSUPPORTED`，Phase 1 Full 未授权 |
