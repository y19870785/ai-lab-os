...
| PILOT-001 | 企业微信 Owner 可信任务捕获规划 | PLANNING_BASELINE_APPROVED / FINAL_INDEPENDENT_PLANNING_REVIEW_PASSED / IMPLEMENTATION_NOT_APPROVED / REQUIRES_SEPARATE_IMPLEMENTATION_AUTHORIZATION / REAL_PILOT_NOT_STARTED |
...
仍未接入。PILOT-001 规划基线已获批准并通过最终独立规划审查，只规划企业微信单 Owner 私聊到既有
UserTask 的可信闭环；实现需要单独授权，真实 Phase 与真实 Pilot 尚未启动。REL-036 未启动。
