...
成功终态必须同时具备：Inbox `COMPLETED`、唯一 Quote linkage、Quote canonical read-back、匹配 revision、Audit correlation 与 Verified Result。失败终态必须保留 stage、稳定 failure code、最后已知 canonical 状态、retryability 和人工恢复指引。
