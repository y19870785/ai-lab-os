...
callback，但同 UID 独立进程能通过 `pidfd_getfd` 复制并调用 anonymous issuer capability。Spike 因
`SIGNING_ORACLE_ISOLATION_FAILED` 停止，最终分类为 `UNSUPPORTED`；没有继续真实 callback、Bridge 或业务实现。
